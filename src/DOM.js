...
const generateTask = () => {
  ...
  confirmTaskBtn.addEventListener("click", function () {
    ...
    const inputs = document.querySelectorAll(
      "#taskName, #taskDate, #prioritySelector"
    );
...
const displayTasks = () => {
  ...
  if (
    projectList.map((object) => object.title).indexOf(currentProject) !== -1
  ) {
    currentProjectHeader.textContent = currentProject;
  } else {
...
const displayProjects = () => {
  ...
  projectList.forEach((element) => {
    ...
    projectDelete.addEventListener("click", function (event) {
      const projectNameTarget =
        event.target.previousElementSibling.textContent.toString();
  ...
  menuBtn.addEventListener('click', () => {
    ...
    if (sidebar.classList.contains('collapsed')) {
      ...
      listContainer.style.marginLeft = '300px';
